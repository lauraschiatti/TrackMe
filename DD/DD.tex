...
When the third-party users have subscribed to the bulk data subscription, they define an update time when they want to get the refreshed data (e.g. update data in every 'X' hours).
	...
	\section{Component view}
		...
		The interfaces provided by D4H can be categorised in Internal and External. The Internal interfaces are those related to the D4H web-site, while the External interfaces are those related to third party components. In the Table \ref{table:d4h_internal_interfaces} more detail is provided regarding the internal interfaces provided by D4H, and in the Table \ref{table:d4h_external_interfaces} more detail is provided about the external interfaces.\\
		
	\begin{table}[H]
		\centering
		\begin{tabular}{l l p{0.5\textwidth}}
			\hline\hline
			\multicolumn{3}{c}{\textbf{Internal interfaces}} \\
			\hline
			\textbf{Method} & \textbf{Path} & \textbf{Description} \\ [0.5ex]
			\hline
			POST & \textbackslash{web}\textbackslash{login} & Let the user to login into the web-site \\
			POST & \textbackslash{web}\textbackslash{logout} & Let the user to logout into the web-site \\
			POST & \textbackslash{web}\textbackslash{individual}\textbackslash{signup} & Let an Individual create a D4H account \\
 			POST & \textbackslash{web}\textbackslash{thirdparty}\textbackslash{signup} & Let a Third Party company to create a D4H account\\
			GET & \textbackslash{web}\textbackslash{search} & Let the user to search a specific user information or a bulk of anonymised information\\
			GET & \textbackslash{web}\textbackslash{request} & Let the user to get all requests sent to him\\
			POST & \textbackslash{web}\textbackslash{request}\textbackslash{:requestId} & Let the user to accept a request\\
			DELETE & \textbackslash{web}\textbackslash{request}\textbackslash{:requestId} & Let the user to reject a request\\
			GET & \textbackslash{web}\textbackslash{subscription} & Let the user to get all the saved subscriptions\\
			POST & \textbackslash{web}\textbackslash{subscription} & Let the user to save a search query\\
			DELETE & \textbackslash{web}\textbackslash{subscription}\textbackslash{:queryId} & Let the user to remove a subscription\\
			\hline
		\end{tabular}
		\caption{Internal interfaces provided by D4H}
		\label{table:d4h_internal_interfaces}
	\end{table}
	...
	\begin{table}[h]
		...
		\begin{tabular}{l l p{0.6\textwidth}}
			\hline\hline
			\multicolumn{3}{c}{\textbf{External interfaces}} \\
			\hline
			\textbf{Method} & \textbf{Path} & \textbf{Description} \\ [0.5ex]
			\hline
			POST & \textbackslash{api}\textbackslash{request} & Let the third party send a request for accessing an individual's health data and location\\
			DELETE & \textbackslash{api}\textbackslash{request}\textbackslash{:requestId} & Let a third party to remove a request sent to an individual\\
			\hline
		\end{tabular}
		\caption{External interfaces provided by D4H}
		\label{table:d4h_external_interfaces}
	\end{table}
	...
	On the other hand, \textbf{ASOSBackend} component sends notifications to the different \textbf{Health Care Service} components using the provided \textbf{Alarm Interface}.
		...
		\end{longtable}
>>>>>>> 711bc7d45006b00cf0128d5c86472d8644d702eb
