...
%	\thispagestyle{empty}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[scale=0.5]{Diagrams/high_level.jpg}
	...
	Next, comes the web page through which users can Login into the system (if already registered). And if not, they can register themselves through the Register web-page. There are separate register forms for the Individual users and for third party users, which can be seen in the Figure \ref{fig:Login}.
	...
	Registration is free for all the users and it doesn't take much time to complete the forms, as can be seen in the Figure \ref{fig:Register} and Figure \ref{fig:Register_third_party}.
	...
	In the Figure \ref{fig:Dashboard_individual}, its personal dashboard for the individual users, who can manage and view the requests and are able to view who all third parties have subscribed to their data.
	...
	In the Figure \ref{fig:Individual_Request}, can be seen that the customer has two options to choose: accept or reject the request for data acquisition. As soon as an action is taken upon the request, it gets deleted from the page.
	...
	In the Figure \ref{fig:Make_Request}, Third party users can make request for the data using the filtering criteria provided by the system.
	...
	\begin{figure}[H]
		...
		\label{fig:create_run}
	\end{figure}
	
	In the Figure \ref{fig:create_run}, Organizer's are able to view the pending invites to send to targeted participants and here, they can start configuring a new run by clicking on 'Create new run' button.
	 ...
	 \begin{figure}[H]
		...
		\label{fig:define_circuit}
	\end{figure}
	
	In the Figure \ref{fig:define_circuit}, Organizer's are able to define the parameters to set up a complete running circuit and here, they can select whether to send invitations now or not by clicking the checkbox.
	...
	\begin{figure}[H]
		...
		\label{fig:available_runs}
	\end{figure}
	
	In the Figure \ref{fig:available_runs}, Spectators are able to view all the available runs, and by clicking on view participants, they are redirected to the map-view of the selected run. 
	...
	\begin{figure}[H]
		...
		\label{fig:view_during_race}
	\end{figure}
	
	In the Figure \ref{fig:view_during_race}, Spectators are able to view the participants' position during the run and time left for the race to end. 
	...
	\begin{figure}[H]
		...
		\label{fig:race_ended}
	\end{figure}
	
	In the Figure \ref{fig:race_ended}, Spectators are able to view an error message when the race has been ended. 
	...
	\begin{itemize}
		...
		\textbf{Event flow}:
		\begin{enumerate}
			\item{The system gets the health-care service contact of the given individual}
