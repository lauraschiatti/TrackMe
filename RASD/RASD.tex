...
\begin{itemize}
  ...
  \item{\textbf{Track4Run}}
  \begin{enumerate}
    \textbf{\goal{6} Run organizers could define the path for a given run and TrackMe users can enroll to it.}
    \begin{enumerate}
...
Finally, the Accept invitation to a run use case (\usecase{21}) is modeled in the Figure \ref{fig:t4r_accept_invitation}. In this case, the Participant asks T4R to show his/her pending invitations, which are collected from the Invitations collection. Once the Participant has all the pending invitations, he/she can select one and accept the invitation, action that will enroll the Participant to the given run, and set the invitation as \textit{Accepted}.
